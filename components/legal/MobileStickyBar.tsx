 ...
 * Three equal-width tap targets with a plain description of what each opens.
 */
  ...
  return (
    <nav className="mbar" aria-label={t("nav")}>
      <a href={`tel:${firm.phoneE164}`} aria-label={t("callAria")}>
        ...
        <span className="mbar-copy">
          <span className="mbar-label">{t("call")}</span>
          <span className="mbar-next">{t("callNext")}</span>
        </span>
      </a>
      <a
      ...
      >
        ...
        <span className="mbar-copy">
          <span className="mbar-label">{t("chat")}</span>
          <span className="mbar-next">{t("chatNext")}</span>
        </span>
      </a>
      <a href={`mailto:${firm.publicEmail}`} aria-label={t("emailAria")}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
          <rect x="3" y="5" width="18" height="14" rx="1" />
          <path d="m3 7 9 6 9-6" />
        </svg>
        <span className="mbar-copy">
          <span className="mbar-label">{t("email")}</span>
          <span className="mbar-next">{t("emailNext")}</span>
        </span>
      </a>
    </nav>
  );
