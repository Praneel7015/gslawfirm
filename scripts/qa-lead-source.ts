...
assert.equal(sourcedLead.source.originPath, "/legal-notices");
assert.equal(
...
assert.equal(
  leadSchema.safeParse({
    ...baseLead,
    source: { originPath: "https://example.com/legal-notices" },
  }).success,
  false,
);
...
assert.equal(
  leadSchema.safeParse({
    ...baseLead,
    source: { originPath: "//example.com/legal-notices" },
  }).success,
  false,
);
...
assert.match(text, /Origin:\s+\/legal-notices/);
...
assert.match(html, /\/legal-notices/);
